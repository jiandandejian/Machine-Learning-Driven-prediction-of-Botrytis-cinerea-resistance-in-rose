...
# --- Model Training Section (e.g., LightGBM Classifier) ---
...
if generated_data_file: #
    ...
    # These parameters can be replaced and adjusted for different models.
    model_param_grid = {
        'n_estimators': [100, 200], # Number of boosting rounds (weak learners)
        'max_depth': [-1, 10, 15], # Maximum tree depth (-1 means no limit)
        'min_child_samples': [20, 50], # Minimum number of data needed in a child (leaf)
        'colsample_bytree': [0.8, 1.0], # Subsample ratio of columns when constructing each tree
        'subsample': [0.8, 1.0], # Subsample ratio of the training instance
        'min_split_gain': [0.1, 0.2], # Minimum gain to make a split
        'boosting_type': ['gbdt', 'dart'], # Boosting type (Gradient Boosting Decision Tree or Dropouts meet Multiple Additive Regression Trees)
    }
    ...
    grid_search = GridSearchCV(
        estimator=LGBMClassifier(random_state=42, verbosity=-1), # Instantiate LightGBM classifier with random state and silent mode
    ...
    # Write model training results to a more generic results file
    # If integrating multiple models, consider naming this file to associate it with the specific model used.
    with open('out_result_Nature_GWAS_LGBMC_Kinship_endadta.txt', 'a') as file:
        print(f"Model_Best_parameters for {generated_data_file}: {best_params}", file=file) #
