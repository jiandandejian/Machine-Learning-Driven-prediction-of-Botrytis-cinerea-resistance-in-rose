...
snp_data = df.iloc[:, 1:-1]  # Exclude sample name and phenotype
...
summary_df = pd.DataFrame({
    ...
    "Cluster Count": [len(cluster_results[t]["clusters"]) for t in thresholds],
    "Clustered Samples Count": [len(cluster_results[t]["clustered"]) for t in thresholds],
    "Ungrouped Samples Count": [len(cluster_results[t]["ungrouped"]) for t in thresholds]
})
...
# --- Model Training Section (MLP Classifier) ---
...
if generated_data_file:
    ...
    MLPC_param_grid = {
        ...
        'max_iter': [300],
        'batch_size': ['auto', 16],
        'random_state': [42]  # Keep random_state for reproducibility in GridSearch
    }
    ...
    grid_search = GridSearchCV(
        estimator=MLPClassifier(random_state=42), # Instantiate MLPClassifier with random state
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        # Initialize and train the MLP Classifier with the best parameters
    ...
    # Write model training results to a specific file for MLP Classifier
    with open('out_result_Nature_GWAS_MLPC_Kinship_endadta.txt', 'a') as file:
        # Output best parameters
        print(f"Nature_GWAS_MLPC_Kinship_Best_parameters: {best_params}", file=file)
