...
from itertools import product # Import product for iterating over hyperparameter combinations
...
for i in range(n_samples): #
    for j in range(i): # Only compute the lower triangle, as the distance matrix is symmetric
        vec_i = snp_data.iloc[i] # Get SNP data for sample i
        vec_j = snp_data.iloc[j] # Get SNP data for sample j
...
with open("Genetic_distance_lower_triangle.txt", "w") as f: #
    ...
    for i in range(1, n_samples): #
        row = " ".join(f"{distance_matrix[i, j]:.5f}" for j in range(i)) # Format distance to 5 decimal places
        f.write(row + "\n") #
...
def cluster_samples_by_threshold(distance_dict, threshold, sample_ids): #
    ...
    for k in valid_pairs: #
        i, j = map(int, k.split('_')) # Parse sample indices
...
summary_df.to_csv("Genetic_distance_summary.csv", index=False) # Save summary statistics to a CSV file
...
for thresh in thresholds: #
    ...
    # Save genotype/phenotype data for this threshold if the number of clustered samples is >= 100
    # The original file had a hardcoded upper limit < 140, removed for flexibility as requested.
    if len(clustered_ids) >= 100: #
        # Construct the output filename without specific model names
        file_name_for_model = f"Genetic_distance_clustered_data_threshold_{suffix}.tsv"
        # Filter the DataFrame to include only the clustered samples
        clustered_df = df[df.iloc[:, 0].isin(clustered_ids)] #
        clustered_df.to_csv(file_name_for_model, sep="\t", index=False) # Save as a TSV file
...
# --- Model Training Section (Logistic Regression Classifier) ---
...
if generated_data_file:
    ...
    data = pd.read_csv(file_path, index_col=0, sep='\t') # Load the data, using the first column as index

    # Prepare feature matrix X and label vector y
    X = data.drop(columns=['rank']) # 'rank' column is the label, the rest are features
    y = data['rank'].map({'R': 0, 'S': 1}) # Map 'R' to 0 and 'S' to 1
    ...
    # Removed 'saga' from solver_list and added `product` for grid generation as in the original LR script
    LR_param_grid = [] #
    C_list = [0.001, 0.01, 0.1, 1, 10, 100] # Inverse of regularization strength
    max_iter_list = [500, 1000, 2000, 3000] # Increased max_iter to reduce convergence warnings
    penalty_list = ['l1', 'l2'] # Specifies the norm used in the penalization
    solver_list = ['liblinear', 'lbfgs'] # Algorithm to use in the optimization problem
    class_weight_list = [None, 'balanced'] # Weights associated with classes

    # Manually generate valid combinations of hyperparameters to avoid conflicts between penalty and solver
    for penalty, C, solver, class_weight, max_iter in product(penalty_list, C_list, solver_list, class_weight_list, max_iter_list): #
        if penalty == 'l1' and solver not in ['liblinear']: # 'l1' penalty only works with 'liblinear' solver
            continue #
        if penalty == 'l2' and solver not in ['liblinear', 'lbfgs']: # 'l2' penalty works with 'liblinear' and 'lbfgs'
            continue #
    ...
    grid_search = GridSearchCV(
        estimator=LogisticRegression(random_state=42), # Instantiate Logistic Regression classifier with random state
    ...
    # Write model training results to a specific file for Logistic Regression
    with open('out_result_Nature_GWAS_LR_Kinship_endadta.txt', 'a') as file: #
        print(f"Nature_GWAS_LR_Kinship_Best_parameters: {best_params}", file=file) #
